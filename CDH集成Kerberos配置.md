...
集群主机角色划分：
* sunmvm20	M RS DN NN G WHCS ICS ID ISS OS SS G HS S2S JHS NM RM
* sunmvm26	HBRESTS HBTS RS B DN G HS2 ID G G NM S
* sunmvm27	RS DN SNN G HMS ID AM AP ES HM SM G NM S
* sunmvm28	RS DN HFS G HS KTR ID G NM S

sunmvm20作为master节点，其他节点作为slave节点，我们在sunmvm20节点安装kerberos Server，在其他节点安装kerberos client。
...
在 sunmvm20 上安装包 krb5、krb5-server 和 krb5-client。
```shell
...
也就是说，如果Hadoop集群存在一个名为hdfs@0HKJ.COM的principal
